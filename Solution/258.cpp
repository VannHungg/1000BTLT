...
void nguyenToTang(int iArr[], int iN){
	int iMangPhu[MAX];
	int iSoLuong = 0;
	for (int i = 0; i < iN; i++){
		if (kiemTraSoNguyenTo(iArr[i])){
			iMangPhu[iSoLuong++] = iArr[i];
		}
	}
	sapXep(iMangPhu, iSoLuong, 't');
	iSoLuong = 0;
	for (int i = 0; i < iN; i++){
		if (kiemTraSoNguyenTo(iArr[i])){
			iArr[i] = iMangPhu[iSoLuong++];
		}
	}
	xuatMang(iArr, iN);
}
...
int main(){
	...
	cout << "nguyen to tang: " << endl;
	nguyenToTang(iArr, iN);
